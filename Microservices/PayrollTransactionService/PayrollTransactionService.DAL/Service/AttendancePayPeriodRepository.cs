...
{
    public class AttendancePayPeriodRepository : IAttendancePayPeriodRepository
    {
        #region Constructor 
        private readonly IDbConnection _dbConnection;
        public AttendancePayPeriodRepository(IDbConnection dbConnection)
        {
        ...
        public async Task<IEnumerable<PeriodRequest>> GetAttendancePayPeriodsByCompanyIdAsync(string procedureName, object parameters)
        {
            return await _dbConnection.QueryAsync<PeriodRequest>(procedureName, parameters, commandType: CommandType.StoredProcedure);
        }
       
        public async Task<(IEnumerable<PeriodRequest> Periods, string ErrorMessage)> GetAttendancePayPeriodsByCompanyIdAndSDateAsync(string procedureName, object parameters)
        {
        ...
        public async Task<PeriodRequest> AddAsync(string procedureName, PeriodRequest model)
        {
            ...
            parameters.Add("@Contractor_Code", model.Contractor_Code, DbType.String);
            parameters.Add("@WorkOrder_Code", model.WorkOrder_Code, DbType.String);
