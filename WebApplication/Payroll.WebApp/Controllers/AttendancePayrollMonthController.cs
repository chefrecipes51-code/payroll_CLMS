...
/// Date:- 26-05-2025
/// Note:- Perform all the necessary operation regarding Attendance Payroll Month 
///         1) Add Attendance Payroll month based on Company,Contractor and work order wise
///         2) View Attendance Payroll month based on company AND custom Group and order wise.
...
namespace Payroll.WebApp.Controllers
{
    ...
    public class AttendancePayrollMonthController : SharedUtilityController
    {
        ...
        public AttendancePayrollMonthController(IConfiguration config, RestApiTransactionServiceHelper transactionServiceHelper, RestApiMasterServiceHelper masterServiceHelper, RestApiUserServiceHelper userServiceHelper, IMapper mapper, IOptions<ApiSettings> apiSettings)
        {
            _mapper = mapper;
        ...
        [HttpPost]
        public async Task<IActionResult> GetAttendancePayMonthBySDate([FromBody] PayrollRequest request)
        {
            var apiKey = await _userServiceHelper.GenerateApiKeyAsync();
            if (apiKey != null && request.CompanyId != null)
            {
                string baseUrl = _apiSettings.PayrollTransactionEndpoints.GetAttendancePayrollMonthBySdateUrl;
                string dateStr = request.SelectedDate?.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);

                string apiUrl = $"{baseUrl}?id={request.CompanyId}&createdBy={SessionUserId}&Contractor_Code={request.Contractor_Code}&WorkOrder_Code={request.WorkOrder_Code}";
        ...
        #region Bind Contractor by Company ID
        [HttpGet]
        public async Task<IActionResult> getcontractorbycompanyid(string companyId)
        {
            if (!string.IsNullOrEmpty(companyId))
            {
                string apiUrl = $"{_apiSettings.PayrollMasterServiceEndpoints.GetCompanyDetailsByIdUrl}/{companyId}";
        ...
        [HttpGet]
		public async Task<IActionResult> GenerateAttendancePayrollMonth()
		{
			return View();
		}
		[HttpPost]
        public async Task<IActionResult> GenerateAttendancePayrollPeriod(string PeriodType, string CompanyId,int ContractorId, string PeriodFDate, string PeriodEDate,string Contractor_Code,string WorkOrder_Code)
        {
            ...
            try
            {
                ...
                {                   
                    ...
                    var apiUrl = _apiSettings.PayrollTransactionEndpoints.PostAttendancePayrollMonthDetailUrl;                   
        ...
        [HttpGet]
        public async Task<IActionResult> GetPayrollMonthGroupListJson(int companyid,string Contractor_code,string Workorder_code)
        {
            var apikey = await _userServiceHelper.GenerateApiKeyAsync(); // GET API KEY 
            if (apikey != null)
            {
                var payrollgroup = await GetPayrollMonthGroupListAsync(companyid, Contractor_code, Workorder_code, apikey);
        ...
        private async Task<List<PeriodRequest>> GetPayrollMonthGroupListAsync(int id,string Contractor_Code,string WorkOrder_Code, string apiKey)
        {
            string apiUrl = $"{_apiSettings.PayrollTransactionEndpoints.GetAttendancePayrollMonthGroupUrl}?id={id}" +
                    $"&contractor_code={Uri.EscapeDataString(Contractor_Code)}" +
                    $"&workorder_code={Uri.EscapeDataString(WorkOrder_Code)}";
        ...
        [HttpGet]
        public async Task<IActionResult> GetPayrollMonthListJson(int companyid,string ContractorCode, string WorkOrderCode, string payrollGroup)
        {
            var apikey = await _userServiceHelper.GenerateApiKeyAsync(); // GET API KEY 
            if (apikey != null)
            {
                ...
                var payrollgroup = await GetPayrollMonthListAsync(companyid, ContractorCode, WorkOrderCode, encodedGroup, apikey);
        ...
        private async Task<List<PeriodRequest>> GetPayrollMonthListAsync(int id,string Contractor_Code, string WorkOrder_Code,string encodedGroup, string apiKey)
        {
            string apiUrl = $"{_apiSettings.PayrollTransactionEndpoints.GetAttendancePayrollMonthGroupUrl}?id={id}&fGroupDate={encodedGroup}&Contractor_Code={Contractor_Code}" +
                    $"&WorkOrder_Code={WorkOrder_Code}";
        ...
        [HttpGet]
		public async Task<IActionResult> AttendancePayrollMonthList()
		{
            //await SetUserPermissions();
            return View();
		}
