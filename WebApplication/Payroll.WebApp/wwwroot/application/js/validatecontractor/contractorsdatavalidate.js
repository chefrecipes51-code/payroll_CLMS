...
$(document).ready(function () {
    ...
    //});
    //$(document).on('change', '#validatecontractors-list .row-checkbox', function () {
    //    const $all = $('#validatecontractors-list .row-checkbox:not(:disabled)');
    //    const $checked = $('#validatecontractors-list .row-checkbox:checked:not(:disabled)');
    //    $('#selectAllContractor').prop('checked', $all.length === $checked.length);
    //});
    ...
    //});
    // Contractor Header Checkbox
    $(document).on('change', '#selectAllContractor', function () {
        const isChecked = $(this).is(':checked');
        $('#validatecontractors-list .row-checkbox:not(:disabled)').prop('checked', isChecked);
    });

    // Contractor Row Checkbox
    $(document).on('change', '#validatecontractors-list .row-checkbox', function () {
        const $all = $('#validatecontractors-list .row-checkbox:not(:disabled)');
        const $checked = $('#validatecontractors-list .row-checkbox:checked:not(:disabled)');
        $('#selectAllContractor').prop('checked', $all.length === $checked.length);
...
function fetchGradeDropdownOptionsOnce() {
    ...
    $(document).ready(function () {
        ...
        $('#validateentity-list').on('change', '.grade-dropdown', function () {
            ...
            const $editableCell = $dropdown.closest('.editable-cell');

            if (selectedVal && selectedVal !== '') {
                $checkbox.prop('disabled', false);
                $editableCell.addClass('grade-cell-updated');
            } else {
                $checkbox.prop('disabled', true);
                $editableCell.removeClass('grade-cell-updated');
            }
        });
    });
...
$(document).on('input', '.compliance-input', function () {
    ...
    if (decimalFields.includes(field)) {
        ...
        const originalCaret = inputEl.selectionStart;

        var raw = val.replace(/\D/g, '').substring(0, 10);
        var formatted = '';
        if (raw.length > 0) formatted += raw.substring(0, Math.min(2, raw.length));
        if (raw.length >= 3) formatted += '-' + raw.substring(2, Math.min(9, raw.length));
        if (raw.length === 10) formatted += '-' + raw.substring(9);

        $input.val(formatted);
        val = formatted;  // ✅ update val so validation works!

        // restore caret
        setTimeout(() => {
            var newCaret = originalCaret;
            if (originalCaret === 2 && raw.length >= 2) newCaret += 1;
            if (originalCaret === 9 && raw.length >= 9) newCaret += 1;
    ...
    // ✅ IMMEDIATE COLOR CHANGE BASED ON VALIDATION
    const $cell = $input.closest('.editable-cell');

    var isFieldValid = true;
    if (!val) {
        isFieldValid = false;
    } else if (field === 'PFNo' && !isValidPFNo(val)) {
        isFieldValid = false;
    } else if (field === 'UANNo' && !isValidUAN(val)) {
        isFieldValid = false;
    } else if (field === 'ESICNo' && !isValidESIC(val)) {
        isFieldValid = false;
    } else if (decimalFields.includes(field) && isNaN(val)) {
        isFieldValid = false;
    }

    // ✅ Apply styling
    if (isFieldValid) {
        $cell.removeClass('bg-light-danger').addClass('grade-cell-updated');
    } else {
        $cell.removeClass('grade-cell-updated').addClass('bg-light-danger');
    }
    ...
    const missingMap = JSON.parse($row.attr('data-missing') || '{}');
    var allMissingFieldsValid = true;

    for (const fieldName in missingMap) {
        if (!missingMap[fieldName]) continue;

        const $cell = $row.find(`.editable-cell[data-col="${fieldName}"]`);
        const $inputField = $cell.find('input');
        var value;
    ...
    $row.find('.compliance-checkbox').prop('disabled', !allMissingFieldsValid);
});
...
function populateMultiSelectDropdown(selector, data, valueField, textField, selectedValues = [], modalId = '#filterModal') {
...
function fetchContractors(companyId) {
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            //populateMultiSelectDropdown('#contractorName', data, 'value', 'text');
            populateMultiSelectDropdown('#contractorName', data, 'value', 'text', [], '#filterModal');
...
function fetchLocations(companyId) {
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            //populateMultiSelectDropdown('#locationName', data, 'value', 'text');
            populateMultiSelectDropdown('#locationName', data, 'value', 'text', [], '#filterModal');
