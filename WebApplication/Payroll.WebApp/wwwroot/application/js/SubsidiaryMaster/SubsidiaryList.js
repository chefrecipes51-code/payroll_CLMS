$(document).ready(function () {
	...
	$(document).on("click", "#getDataButton", function (e) {
		...
		const urlParams = new URLSearchParams(window.location.search);
		const encryptedCompanyId = urlParams.get('companyId');
		...
		var redirectUrl = `/SubsidiaryMaster/AddSubsidiaryMaster?subsidiary_Id=${subsidiaryId}`;
		if (encryptedCompanyId) {
			redirectUrl += `&companyId=${encodeURIComponent(encryptedCompanyId)}`;
		}
