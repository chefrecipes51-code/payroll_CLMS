    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.isSuccess && response.result) {
                ...
                // Reset and set placeholder for all dropdowns
                cityDropdown.empty().append(`<option value="">Select City</option>`).trigger('change');
                locationDropdown.empty().append(`<option value="">Select Location</option>`).trigger('change');
                roleDropdown.empty().append(`<option value="">Select Role</option>`).trigger('change');
                ...
                cityDropdown.off("change").on("change", function () {
                    ...
                    if (selectedCityId && data.locations && data.locations.length > 0) {
