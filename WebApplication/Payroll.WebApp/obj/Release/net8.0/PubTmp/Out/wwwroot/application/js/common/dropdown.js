...
$(document).ready(function () {
    ...
    const dropdownConfigs = [
        ...
        { selector: '#SubsidiaryCompany', url: '/DropDown/FetchCompaniesDropdown', placeholder: 'Select Company', valueKey: 'value', textKey: 'text', autoSelectFirst: false }, //added by krunali payroll 431
        { selector: '#SubsidiaryCountry', url: '/DropDown/FetchCountriesDropdown', placeholder: 'Select Country', valueKey: 'value', textKey: 'text', autoSelectFirst: false }, //added by krunali payroll 431
        { selector: '#SubsidiaryType', url: '/DropDown/FetchSubsidiaryTypesDropdown', placeholder: 'Select Subsidiary', valueKey: 'value', textKey: 'text', autoSelectFirst: false },//added by krunali payroll 431
...
/* #endregion */
///////////////////////////////////////////////////////// Region for Krunali:- Start///////////////////////////////////////////////////////
$(document).ready(function () {
    ////added by krunali payroll 431
    $('#SubsidiaryCountry').change(function () {
        const countryId = $(this).val();
        if (countryId) {
            //console.log("Selected Country ID:", countryId);
            fetchDropdownData('/DropDown/FetchStateDropdown/' + countryId, { Country_Id: countryId }, '#SubsidiaryState', 'Select State');
            clearDropdowns(['#SubsidiaryCity', '#SubsidiaryLocation'], 'Select City/Location');
        } else {
            clearDropdowns(['#SubsidiaryState', '#SubsidiaryCity', '#SubsidiaryLocation', '#SubsidiaryArea'], 'Select Option');
        }
    });
    //added by krunali payroll 431
    $('#SubsidiaryState').change(function () {
        const stateId = $(this).val();
        if (stateId) {
            //console.log("Selected State ID:", stateId);
            clearDropdowns(['#SubsidiaryCity'], 'Select City');
            fetchDropdownData('/DropDown/FetchCityDropdown/' + stateId, { State_Id: stateId }, '#SubsidiaryCity', 'Select City');
        }
    });
    //added by krunali payroll 431
    $('#SubsidiaryCity').change(function () {
        const cityId = $(this).val();
        if (cityId) {
            //console.log("Selected City ID:", cityId);
            clearDropdowns(['#SubsidiaryLocation'], 'Select location');
            fetchDropdownData('/DropDown/FetchLocationsDropdown/' + cityId, { City_Id: cityId }, '#SubsidiaryLocation', 'Select Location');
        }
    });
    //added by krunali payroll 431
    $('#SubsidiaryLocation').change(function () {
        const locationId = $(this).val();
        if (locationId) {
            console.log("Selected Location ID:", locationId);
            clearDropdowns(['#SubsidiaryArea'], 'Select Area');
            fetchDropdownData('/DropDown/FetchAreaDropdown/' + locationId, { Location_Id: locationId }, '#SubsidiaryArea', 'Select Area');
        }
    });
});
///////////////////////////////////////////////////////// Region for Krunali:- END///////////////////////////////////////////////////////
