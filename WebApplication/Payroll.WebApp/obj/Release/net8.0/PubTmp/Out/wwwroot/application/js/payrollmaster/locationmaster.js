$(document).ready(function () {
    ...
    $(document).on('click', '#resetLocationButton', function () {
        const isEditMode = $('#locationId').val() !== ''; // Check if it's Edit mode

        if (isEditMode) {
            // Restore original values from the response
            if (originalLocationFormData) {
                ...
                $('#locationCityDropdown').val(originalLocationFormData.CityId);
                ...
                $('#toggleContainer').show(); // Ensure toggle is visible
            }
        } else {
