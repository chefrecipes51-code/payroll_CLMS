...
function validateCompanyAddress() {
    ...
    if (/^\s|\s$/.test(rawCompanyAddress)) {
        $("#CompanyAddress-error").text("Please Provide Proper Company Address.");
        return false;
    }
...
//    var buildingNo = rawBuildingNo.trim(); // Trimmed value for validation
...
//        $("#Building-No-error").text("Please Provide Proper Building No.");
//        return false;
//    }
...
//        return false;
//    } else if (buildingName.length > 100) {
//        $("#Building-Name-error").text("Building Name cannot exceed 100 characters.");
//        return false;
//    }
//    $("#Building-Name-error").text(""); // Clear error if valid
//    return true;
//}
//function validateBuildingName() {
//    var rawBuildingName = $('#Building_Name').val(); // Get raw input
//    var buildingName = rawBuildingName.trim(); // Trimmed version for validation
...
//        $("#Building-Name-error").text("Please Provide Proper Building Name.");
//        return false;
//    }
...
function validateStreet() {
    ...
    if (/^\s|\s$/.test(objStreet)) {
        $("#Street-error").text("Please Provide Proper Street.");
        return false;
    }
...
//    var primaryPhoneNo = $('#Primary_Phone_No').val().trim();
...
//        $("#Primary-Phone-No-error").text("Please enter a valid 10-digit to 11-digit phone number.");
//        return false;
//    }
...
function validatePrimaryPhoneNo() {
    ...
    if (/^\s|\s$/.test(rawPrimaryPhoneNo)) {
        $("#Primary-Phone-No-error").text("Please Provide Proper Primary Phone No.");
        return false;
    }
...
//    var secondaryPhone = $('#Secondary_Phone_No').val().trim();
//    var phoneRegex = /^[0-9]{10,11}$/; // Ensures exactly 10 digits

//    $("#Secondary-Phone-No-error").text(""); // Clear previous error message

//    if (secondaryPhone !== "" && !phoneRegex.test(secondaryPhone)) {
//        $("#Secondary-Phone-No-error").text("Please enter a valid 10-digit to 11-digit phone number.");
//        return false;
//    }

//    return true; // If valid or empty, return true
//}
function validateSecondaryPhoneNo() {
    ...
    if (rawSecondaryPhone !== "") {
        if (/^\s|\s$/.test(rawSecondaryPhone)) {
            $("#Secondary-Phone-No-error").text("Please Provide Proper Secondary Phone Number.");
            return false;
        }
...
//    var primaryEmail = $('#Primary_Email_Id').val().trim();
//    //var emailRegex = /^(?![\s.])([^\s@]+@[^\s@]+\.[^\s@]+)$/; // No leading spaces or dots
//    var emailRegex = /^[^\s@][^\s@]*@[^\s@]+\.[^\s@]+$/; // Ensures first char is not space or '@'
...
//    if (/^[\s.@]/.test(rawEmail)) {
//        $("#Primary-Email-Id-error").text("Please Provide Proper Primary Email ID.");
//        return false;
//    }
...
//    var secondaryEmail = $('#Secondary_Email_ID').val().trim();
//    //var emailRegex = /^(?![\s.])([^\s@]+@[^\s@]+\.[^\s@]+)$/; // No leading spaces or dots
//    var emailRegex = /^[^\s@][^\s@]*@[^\s@]+\.[^\s@]+$/; // Ensures first char is not space or '@'

//    $("#Secondary-Email-ID-error").text(""); // Clear previous errors

//    if (secondaryEmail !== "") {  // Validate only if value is entered
...
//        }
//    }
//    return true;  // Valid if empty or matches regex
//}
...
function validateSecondaryEmailId() {
    ...
    if (rawSecondaryEmail !== "") {  // Validate only if value is entered
        if (/^[\s.@]/.test(rawSecondaryEmail)) {
            $("#Secondary-Email-ID-error").text("Please Provide Proper Secondary Email ID.");
            return false;
        }
        // Check if raw input starts with whitespace, dot, or @
        if (/^[\s.@]/.test(rawSecondaryEmail)) {
...
function validateWebsiteUrl() {
    ...
    if (companyWebsiteUrl !== "") {
        ...
        if (!urlRegex.test(companyWebsiteUrl)) {
            $("#Website-url-error").text("Please enter a valid Website URL (e.g., https://example.com).");
            return false;
        }
    }
