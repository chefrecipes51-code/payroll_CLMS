    ...
    if (userData) {
        ...
        $("#userRoleLocation").on("change", function () {
            ...
            if (storedData) {
                ...
                roleLocationData.forEach(mapping => {
                    if (mapping.locationId === selectedLocationId) {
                        // Ensure roleMenuHdrId is used correctly
                        roleDropdown.append(`<option value="${mapping.roleMenuHdrId}">${mapping.roleName}</option>`);
                    }
                });

                roleDropdown.trigger("change"); // Trigger role selection change
            }
        });
